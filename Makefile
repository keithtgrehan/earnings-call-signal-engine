...
acquisition-discover-ir:
	$(PYTHON) tools/discover_company_ir_sources.py

acquisition-discover-paid-api:
	$(PYTHON) tools/discover_paid_transcript_api_sources.py

transcript-normalize-dry-run:
	PYTHONPATH=src $(PYTHON) -m signal_engine.transcripts.normalize --dry-run

rag-chunks-dry-run:
	$(PYTHON) tools/build_nyse_100_rag_chunks.py --workspace /tmp/signal-engine-rag-chunks-dry-run --audit /tmp/signal-engine-rag-chunks-dry-run/_audit/missing.csv --out-manifest /tmp/signal-engine-rag-chunks-dry-run/nyse_100_chunk_manifest.csv --desktop-index /tmp/signal-engine-rag-chunks-dry-run/rag_chunk_index.csv
