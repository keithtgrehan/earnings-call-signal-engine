...
def write_classification_outputs(
    *,
    ...
    for unit_type, rows in outputs_by_unit.items():
        ...
        path = target_dir / _CLASSIFICATION_FILE_NAMES[unit_type]
        _write_jsonl(path, unit_records)
        artifacts[unit_type] = path
        summary["unit_counts"][unit_type] = len(rows)
        summary["label_distributions"][unit_type] = top_label_counts
...
def write_embedding_outputs(
    *,
    ...
    for unit_type, rows in outputs_by_unit.items():
        ...
        path = target_dir / _EMBEDDING_FILE_NAMES[unit_type]
        _write_jsonl(path, unit_records)
        artifacts[unit_type] = path
        summary["unit_counts"][unit_type] = len(rows)
        summary["vector_dimensions"][unit_type] = dimension

        similarity_payload = similarity_by_unit.get(unit_type, {})
        if similarity_payload:
            similarity_path = target_dir / _SIMILARITY_FILE_NAMES[unit_type]
            _write_json(similarity_path, similarity_payload)
            artifacts[f"{unit_type}_similarity"] = similarity_path
