        ...
        json.dumps(
            {
                ...
                "moments": [{"moment_id": "m01", "caveat": "Transcript-first output remains canonical."}],
            },
            indent=2,
        ),
        encoding="utf-8",
...
def test_validate_reference_case_package_flags_missing_per_moment_caveat(tmp_path: Path) -> None:
    package_dir = tmp_path / "case"
    package_dir.mkdir()
    prefix = "meta"
    ...
    assert any("non-empty per-moment `caveat`" in error for error in errors)
