...
- trigger_frequency: `{'recovery plan': 1, 'fixed': 1, 'send': 1, 'review': 1}`

| id | confusion | triggers | text |
| --- | --- | --- | --- |
| `unc_account_if_recovery_lands_001` | `uncertainty_hedging->opportunity_commitment` | `recovery plan, fixed` | if the recovery plan lands and the integrations are fixed |
...
- trigger_frequency: `{'procurement': 1, 'implementation': 1, 'pilot': 1, 'rollout': 1}`
...
| `neut_sales_opening_full_001` | `neutral->opportunity_commitment` | `pilot, rollout` | thanks for making time i can walk through the pilot scope rollout plan and how teams usually evaluate the workflow |
