...
from signal_engine.audio.asr_backends import detect_local_asr_backend, faster_whisper_model_status, ffmpeg_status  # noqa: E402
...
def check_local_asr_environment(*, workspace: Path = DESKTOP_WORKSPACE) -> dict[str, Any]:
    ...
    model_status = faster_whisper_model_status(workspace=workspace, repo_root=ROOT)
    available = [name for name, row in backends.items() if row.get("dependency_status") in {"available", "available_python_package"}]
    faster_whisper_ready = "faster-whisper" in available and model_status.get("model_status") == "available"
    summary = {
        "ffmpeg": ffmpeg,
        "backends": backends,
        "faster_whisper_model": model_status,
        ...
        "asr_ready_status": "ready" if faster_whisper_ready and ffmpeg.get("ffmpeg_status") == "available" else ("dependency_missing_model" if "faster-whisper" in available else "dependency_missing"),
...
def write_report(summary: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        f"- ASR ready status: `{summary['asr_ready_status']}`",
        f"- ffmpeg: `{summary['ffmpeg'].get('ffmpeg_status')}`",
        f"- ffprobe: `{summary['ffmpeg'].get('ffprobe_status')}`",
        f"- faster-whisper model: `{summary['faster_whisper_model'].get('model_status')}` `{summary['faster_whisper_model'].get('model_path')}`",
