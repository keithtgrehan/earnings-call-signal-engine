...
from signal_engine.training.sources import validate_training_source_rows


def _rows(payload: Any) -> list[dict[str, Any]]:
    rows = payload.get("sources") if isinstance(payload, dict) else payload
    if not isinstance(rows, list) or not all(isinstance(row, dict) for row in rows):
        raise ValueError("NLP training sources must be a list or object with sources list.")
    return rows
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate NLP training-source guardrails.")
    ...
    if summary["errors"]:
        print(f"NLP training-source validation failed: {len(summary['errors'])} error(s).")
        for error in summary["errors"]:
            print(f"- {error}")
        return 1
    print(f"NLP training-source validation passed: {summary['row_count']} row(s).")
    return 0
