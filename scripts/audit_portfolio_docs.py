...
def main(argv: list[str] | None = None) -> int:
    ...
    for relative_path in AUDIT_FILES:
        ...
        if CANONICAL_CASE_ID not in text:
        ...
        for index, line in enumerate(text.splitlines(), start=1):
            if (
                CANONICAL_CASE_ID in line or "canonical demo" in line.lower()
            ) and PLANNED_TOKENS.search(line):
                failures.append(
                    f"{relative_path}:{index}: canonical demo described as planned/future language"
                )
