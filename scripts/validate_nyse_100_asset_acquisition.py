...
def validate_audit_rows(
    ...
    *,
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if row.get("download_status", "") not in VALID_DOWNLOAD_STATUS:
            errors.append(f"{prefix}: invalid download_status {row.get('download_status')!r}")
        if row.get("download_status") == "blocked" and not row.get("blocked_reason"):
        ...
        if asset_type == "transcript" and row.get("download_status") == "downloaded" and row.get("rights_status") not in DOWNLOAD_ALLOWED_RIGHTS:
            errors.append(f"{prefix}: downloaded transcript has disallowed rights_status")
        if asset_type == "audio" and row.get("download_status") == "downloaded" and row.get("rights_status") not in DOWNLOAD_ALLOWED_RIGHTS:
            errors.append(f"{prefix}: downloaded audio has disallowed rights_status")
        if row.get("download_status") == "downloaded" and (is_youtube(row.get("source_url", "")) or row.get("source_type") == "youtube_metadata_only"):
        ...
        if local_path and row.get("download_status") == "downloaded" and not Path(local_path).exists():
