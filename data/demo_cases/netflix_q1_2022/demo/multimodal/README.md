...
This folder is the persistent, reference-quality support pack for the fixed Netflix demo case.
...
- current visual context is heuristic fallback only when model-backed scoring is unavailable

Interpretation rules:
- start with the transcript-backed deterministic docs, not the sidecars
- a same-direction sidecar read is bounded comparison context, not proof
- a disagreement hotspot is a review priority, not a winner or loser
- audio is limited to measured pause, filler, and qualification cues
- visual is bounded on-camera context only and does not change the transcript-backed read
...
- `netflix_visual_support.json`: bounded heuristic visual context for the same timed windows when quality was usable
...
For the committed review bundle, the exact requested local MP4 path did not match. A fallback local Netflix MP4 was found and used for the bounded supporting-only visual pass, and the resulting visual layer stayed in heuristic fallback mode rather than model-backed scoring.
